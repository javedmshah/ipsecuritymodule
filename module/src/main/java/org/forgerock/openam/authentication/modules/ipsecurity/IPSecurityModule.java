...
public class IPSecurityModule extends AMLoginModule {
    ...
    public int process(Callback[] callbacks, int state) throws LoginException {
    	...
    	int nextState = ISAuthConstants.LOGIN_SUCCEED;
         ...
         case 3:
        	 ...
        	 throw new AuthLoginException("IP Address blocked!");
         case 2:
        	 System.out.println("state 2");
        	 javax.security.auth.callback.NameCallback userCallBack = (javax.security.auth.callback.NameCallback) callbacks[0];
        	 defaultUser = new String(userCallBack.getName());
        		 ...
        		 Properties prop = new Properties();
        			...
        			try {
        				...
        				if(xfwd.equalsIgnoreCase(iplist)) {
        					nextState = 2;
        					System.out.println("IP check OK!- will ask for username next");
        					if (debug.messageEnabled()) {
        				...
        				ex.printStackTrace();
        				throw new AuthLoginException("Could not validate IP Address!!");
        			} finally {
        				if (input != null) {
        					try {
        						input.close();
    ...
    public Principal getPrincipal() {
        return new IPSecurityModulePrincipal(defaultUser);
    }
